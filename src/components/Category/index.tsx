      ...
      <div className="flex  gap-4 overflow-x-auto scroll-smooth  p-4">
        {categories.map((category) => (
          <button
            ...
            key={category.id}
            className="font-bold  px-12 py-4 text-base leading-6 text-semiwhite bg-royalblue  rounded-lg shadow-md  flex justify-center items-center focus:bg-cyberyellow focus:text-royalblue"
          >
            {category.name}
          </button>
        ))}
        <button
          onClick={() =>
            setCategories((state) => [
              ...state,
              { name: "Nova Categoria", id: state.length + 1 },
            ])
          }
          className="bg-royalblue shadow-md  rounded-lg p-4 flex items-center text-semiwhite font-bold  "
        >
